...
params:
  results:
    label: "Select the results file"
    value: "../../Analysis/smoothers.Rdata"
    input: file
references:
- id: LindgrenRue2015
...
We set a penalised complexity prior for the precision: $P(\sigma_r > 0.2) = 0.01$ [@SorbyeRue2016].
...
We set a penalised complexity prior for the correlation: $P(\rho > 0.9) = 0.9$ [@SorbyeRue2016].
...
We set a penalised complexity prior for the precision: $P(\sigma_s > 1) = 0.05$ [@SorbyeRue2016].
...
This is the range of the spatial autocorrelation: the distance at which the correlation between two location drops below $0.1$.
...
ggplot(be_segment) + geom_sf(data = be_border, fill = inbo_hoofd) + geom_sf()
```

```{r dutch-mesh, fig.cap = "Mesh for the Dutch data."}
ggplot(nl_segment) + geom_sf(data = nl_border, fill = inbo_hoofd) + geom_sf()
```

```{r great-britain-mesh, fig.cap = "Mesh for the British data."}
ggplot(gb_segment) + geom_sf(data = gb_border, fill = inbo_hoofd) +
  geom_sf() +
  coord_sf(
    xlim = st_bbox(gb_segment)[c(1, 3)], ylim = st_bbox(gb_segment)[c(2, 4)]
  )
```
...
```{r order-trend-10}
lincomb %>%
  filter(duration == "10") -> lc
lc %>%
  distinct(country, species) %>%
  mutate(id = row_number()) -> country_species
expand.grid(
  id.x = country_species$id,
  id.y = country_species$id
) %>%
  inner_join(country_species, by = c("id.x" = "id")) %>%
  inner_join(country_species, by = c("id.y" = "id")) %>%
  inner_join(
    lc %>%
      select(country, species, center, median),
    by = c("country.x" = "country", "species.x" = "species")
  ) %>%
  inner_join(
    lc %>%
      select(country, species, center, median),
    by = c("country.y" = "country", "species.y" = "species", "center")
  ) %>%
  mutate(delta = median.x - median.y) %>%
  group_by(id.x, id.y) %>%
  summarise(distance = sqrt(mean(delta ^ 2)), .groups = "drop") %>%
  pivot_wider(names_from = id.y, values_from = distance) %>%
  select(-id.x) %>%
  as.matrix() -> trend_distance
mds <- monoMDS(trend_distance, k = 1, model = "loc")
country_species %>%
  mutate(mds = mds$points[, 1]) %>%
  inner_join(lc, by = c("country", "species")) %>%
  select(country, species, center, change, mds) -> order_trend_10

lincomb %>%
  filter(duration == 10) %>%
  select(country, species, center, median) %>%
  pivot_wider(names_from = center, values_from = median) %>%
  as.data.frame() -> lc
rownames(lc) <- paste(lc$country, lc$species, sep = "_")
lc %>%
  select(-country, -species) %>%
  as.matrix() %>%
  vegdist(method = "euclidean", na.rm = TRUE) -> trend_dist
trend_dist %>%
  monoMDS(model = "local", k = 1) %>%
  scores() %>%
  as.data.frame() %>%
  ...
  inner_join(
    lincomb %>%
      filter(duration == 10) %>%
      select(country, species, center, change) %>%
      mutate(id = paste(country, species, sep = "_")),
    by = "id"
  ) %>%
  mutate(id = reorder(id, MDS1)) -> order_trend_10
```

```{r trend-table, fig.cap = "Linear trends in 10 year moving windows", fig.height = 7}
man_colt <- c(rev(traffic_palette(7)), "grey75", "grey25", "grey50")
man_colt[4] <- inbo_steun_blauw
names(man_colt) <- c("++", "+", "+~", "~", "-~", "-", "--", "?+", "?-", "?")
ggplot(order_trend_10, aes(y = id, x = center, colour = change)) +
...
```{r trend-dendrogram, fig.cap = "Hierarchical clustering of species based on their ten year moving trends."}
hclust(trend_dist) %>%
...
hyperpar %>%
  ...
  geom_vline(xintercept = 0.2, linetype = 4) +
  geom_errorbarh(position = position_dodge(width = 0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
...
hyperpar %>%
  ...
  geom_vline(xintercept = 0.9, linetype = 4) +
  geom_errorbarh(position = position_dodge(width = 0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
