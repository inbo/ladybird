...
## Overview of linear trends in a moving window

```{r order-trend-10}
lincomb %>%
  select(center, duration, median, species, country) %>%
  pivot_wider(names_from = c(center, duration), values_from = median) %>%
  as.data.frame() -> lc
...
trend_dist %>%
  ...
  inner_join(
    lincomb %>%
      select(country, species, center, change, duration) %>%
      mutate(id = paste(country, species, sep = "_")),
    by = "id"
  ) %>%
  mutate(id = reorder(id, MDS1)) -> order_trend
```

```{r trend-table-10, fig.cap = "Linear trends in 10 year moving windows", fig.height = 7}
man_colt <- c(rev(traffic_palette(7)), "grey75", "grey25", "grey50")
man_colt[4] <- inbo_steun_blauw
names(man_colt) <- c("++", "+", "+~", "~", "-~", "-", "--", "?+", "?-", "?")
order_trend %>%
  filter(duration == "10") %>%
  ggplot(aes(y = id, x = center, colour = change)) +
...
```{r trend-table-10-c, fig.cap = "Linear trends in 10 year moving windows, split by country", fig.height = 8}
order_trend %>%
  filter(duration == "10") %>%
  ggplot(aes(y = species, x = center, colour = change)) +
  geom_point(size = 5, aes(colour = change), show.legend = FALSE) +
  geom_text(aes(label = change), show.legend = FALSE, colour = "white") +
  scale_colour_manual(values = man_colt) +
  theme(axis.title = element_blank()) + 
...
```{r trend-table-5, fig.cap = "Linear trends in 5 year moving windows", fig.height = 7}
order_trend %>%
  filter(duration == "5") %>%
  ggplot(aes(y = id, x = center, colour = change)) +
  geom_point(size = 5, aes(colour = change), show.legend = FALSE) +
...
```{r trend-table-5-c, fig.cap = "Linear trends in 5 year moving windows, split by country", fig.height = 8}
order_trend %>%
  filter(duration == "5") %>%
  ggplot(aes(y = species, x = center, colour = change)) +
  geom_point(size = 5, aes(colour = change), show.legend = FALSE) +
...
```{r trend-table-2, fig.cap = "Linear trends in 2 year moving windows", fig.height = 7}
order_trend %>%
  filter(duration == "2") %>%
  ggplot(aes(y = id, x = center, colour = change)) +
  geom_point(size = 5, aes(colour = change), show.legend = FALSE) +
...
```{r trend-table-2-c, fig.cap = "Linear trends in 2 year moving windows, split by country", fig.height = 8}
order_trend %>%
  filter(duration == "2") %>%
  ggplot(aes(y = species, x = center, colour = change)) +
  geom_point(size = 5, aes(colour = change), show.legend = FALSE) +
...
```{r trend-dendrogram, fig.cap = "Hierarchical clustering of species based on their moving trends for 2, 5, and 10 years.", fig.height = 7}
hclust(trend_dist) %>%
