...
map(secondary_models, "roc") %>%
  ...
  map2(sec_rw_order, ~mutate(.x, rw_order = .y)) %>%
  bind_rows() %>%
  mutate(model = "occurrence") -> sec_auc
```

```{r load-cumulative-models}
dirname(params$models) %>%
  file.path("..", "cumulative") %>%
  list.files(full.names = TRUE) %>%
  map(readRDS) -> cumulative_models
cum_species <- map_chr(cumulative_models, "species")
cum_waic <- map_dbl(cumulative_models, "waic")
map_lgl(cumulative_models, "first_order") %>%
  ifelse(1, 2) -> cum_rw_order
map(cumulative_models, "fixed") %>%
  map(rownames_to_column, var = "parameter") %>%
  map2(cum_species, ~mutate(.x, species = .y)) %>%
  map2(cum_rw_order, ~mutate(.x, rw_order = .y)) %>%
  bind_rows() -> cum_fixed
map(cumulative_models, "hyperpar") %>%
  map(rownames_to_column, var = "parameter") %>%
  map2(cum_species, ~mutate(.x, species = .y)) %>%
  map2(cum_rw_order, ~mutate(.x, rw_order = .y)) %>%
  bind_rows() %>%
  mutate(model = "cumulative") -> cum_hyperpar
map(cumulative_models, "trend") %>%
  map2(cum_species, ~mutate(.x, species = .y)) %>%
  map2(cum_rw_order, ~mutate(.x, rw_order = .y)) %>%
  bind_rows() %>%
  mutate(model = "cumulative") -> cum_trend
map(cumulative_models, "roc") %>%
  map("ci") %>%
  map(
    ~data.frame(parameter = c("lcl", "estimate", "ucl"), auc = as.vector(.x))
  ) %>%
  ...
  map2(cum_rw_order, ~mutate(.x, rw_order = .y)) %>%
  bind_rows() %>%
  mutate(model = "cumulative") -> cum_auc
```

```{r combined}
trend %>%
  mutate(type = "") %>%
  bind_rows(sec_trend, cum_trend) %>%
  mutate(prediction = interaction(model, type, sep = " ")) -> all_trend
hyperpar %>%
  bind_rows(sec_hyperpar, cum_hyperpar) -> all_hyperpar
auc %>%
  bind_rows(sec_auc, cum_auc) -> all_auc
```
...
data.frame(species, waic, rw_order = rw_order, model = "base") %>%
  bind_rows(
    ...
    data.frame(
      species = cum_species, waic = cum_waic, rw_order = cum_rw_order,
      model = "cumulative"
    )
  ) %>%
  group_by(species) %>%
...
```{r cum-fixed, fig.cap = "Parameter estimate when using the cumulative predicted occurrence of H. axyridis as fixed effect covariate."}
cum_fixed %>%
  filter(parameter == "secondary") %>%
  mutate(
    species = reorder(species, mean),
...
- **without**: prediction assuming the predicted occurrence of _H. axyridis_ is 0.1%
